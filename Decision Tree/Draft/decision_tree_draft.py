...
class TreeNode:
    def __init__(self, children=None, entropy=0, depth=0):
        if children is None:
            children = {}
...
class DecisionTreeGenerate:
    ...
    def tree_generate_new(self, node, attribute_value=None, parent_node=None):
        ...
        if not continuous_value:
            ...
            split_sub_dataset, split_sub_label, split_sub_dataset_id = \
                self.split_dataset(sub_dataset, sub_label, attribute_index, continuous_value,
        ...
        for i in keys:
            ...
            if depth <= self.max_depth:
                ...
                new_node.set_parent_attribute(node.get_split_attribute)
                new_node.set_parent_attribute_value(attribute_value)
                ...
                if len(deduplication_label) == 1:
                    ...
                    # new_node.set_instances_id(split_sub_dataset_id[i])
        ...
        #         details[i]['sub_label'] = split_sub_label[i]
        #         node.set_instances_id(i, split_sub_dataset_id[i])
        #         # print(node.get_instances_id)
        #         new_node = TreeNode()
        #         new_node.set_node(split_attribute=self.attributes[attribute_index],
        #                           split_attribute_index=attribute_index,
        #                           details={i: details[i]})
        #         print('node.get_split_attribute: ', node.get_split_attribute)
        #         new_node.set_parent_attribute(node.get_split_attribute)
        #         new_node.set_parent_attribute_value(attribute_value)
        #         new_node.set_attribute_value(i)
        #         deduplication_label = set(split_sub_label[i])
    ...
    def calculate_information_gain(self, dataset, label, attributes, attribute_index,
        ...
        if not is_continuous:
            ...
            sub_label = self.split_dataset(dataset, label, attribute_index, split_attribute_value=attribute_value, is_continuous=True)[1]
            instances_num = 0
            for k in sub_label.keys():
                instances_num += len(sub_label[k])
            if attribute_value not in dict_attributes[attributes[attribute_index]].keys():
                dict_attributes[attributes[attribute_index]][attribute_value] = {}
                dict_attributes[attributes[attribute_index]][attribute_value]['info_gain'] = self.calculate_information_entropy(label)
            # for k in label.keys():
            for k in sub_label.keys():

                dict_attributes[attributes[attribute_index]][attribute_value][k] = self.calculate_information_entropy(sub_label[k])
                dict_attributes[attributes[attribute_index]][attribute_value]['info_gain'] -= \
                    len(sub_label[k]) / instances_num * dict_attributes[attributes[attribute_index]][attribute_value][k]
    ...
    def ID3_choose_optimal_attribute(self, dataset, label, attributes):
        ...
        for attribute_index in range(len(attributes)):
            ...
            if self.is_continuous(attribute_values_list[0]):
                ...
                dict_attributes[attributes[attribute_index]]['info_gain'] = -1
                ...
                for i in attribute_values_list_continuous:
                    dict_attributes = self.calculate_information_gain(dataset, label, attributes, attribute_index,
                                                                      i, dict_attributes, True)[1]
                max_info_gain = -1
                max_value_inner = 0
                for i in dict_attributes[attributes[attribute_index]].keys():
                    if i != 'info_gain':
                        if dict_attributes[attributes[attribute_index]][i]['info_gain'] > max_info_gain:
                            max_info_gain = dict_attributes[attributes[attribute_index]][i]['info_gain']
                            max_value_inner = i
                dict_attributes[attributes[attribute_index]]['info_gain'] = max_info_gain
                # print(dict_attributes)
                # print(max_value)
                continuous_value.append(max_value_inner)
